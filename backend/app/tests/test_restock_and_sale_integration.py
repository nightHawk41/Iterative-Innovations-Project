...
Scenario:
  ...
  3.  POST a restock to A1 (+4 units, new far-future expiry).
  ...
  9.  Verify that attempting to over-sell raises the correct Error (D-2 path).
...
def run_integration_test() -> None:
    ...
    with app.app_context():
        ...
        # ------------------------------------------------------------------ #
        # Changed to 4 so 6 + 4 = 10 (Fits perfectly into max capacity!)
        restock_qty  = 4 
        ...
        # ------------------------------------------------------------------ #
        # Step 6 — Multiple sales drain stock; ValueError on 0 qty          #
        # ------------------------------------------------------------------ #
        # Drain the remaining stock via service calls directly (no CSV needed).
        remaining = slot_after_sale.quantity   # e.g. 9
        ...
        # Now a further sale must raise an error (ValueError or ConcurrencyError).
        raised_error = False
        try:
        ...
        # We check for ValueError because that is what our updated service uses to block < 0
        except (ValueError, ConcurrencyError) as exc:
            raised_error = True
            print(f"✔  [Step 6] Error correctly raised for 0-stock slot:")
            print(f"           '{exc}'")

        assert raised_error, (
            "Expected Error when selling from a 0-stock slot, "
            "but no error was raised (Safeguard not working)."
        )
