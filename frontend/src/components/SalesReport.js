...
export function buildSalesReportHtml(data) {
  const generatedAt = data.generated_at
    ? new Date(data.generated_at).toLocaleString("en-US")
    : new Date().toLocaleString("en-US");

  const source = data.source || "transaction_log";
...
export async function generateSalesReport({ onError } = {}) {
  try {
    const response = await fetch("/api/reports/sales");
    const data = await response.json();

    if (!response.ok) {
      const message = data?.error || "Failed to generate sales report.";
      onError?.(message);
      return false;
    }
    ...
    onError?.(error.message || "Failed to generate sales report.");
    return false;
  }
}